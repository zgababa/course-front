...
const {removeProductsOutOfBudget, closet, totalPrice, productsWithPriceInPercent} = require('./priceProduct');
const {isShowMilkInsideProducts, isShowVegetarienProducts, isShowVegetalienProducts,
  isShowCookedMealProducts} = require('./selectProduct');
...
async function getAllowedSelectedProducts(ctx, info) {
  ...
  const forbiddenProductIds = userProducts.forbidden.reduce((a, b) => a.concat(b), []).map(a => a.id);
  const allowedSelectedProductIds = _.differenceWith(selectedProductIds, forbiddenProductIds, _.isEqual);
...
async function getProductsFromCart(root, args, ctx, info) {
  ...
  const { weeklyBudget, priceQualityProfile } = await ctx.db.query.user({where: {id: userId}}, `{weeklyBudget priceQualityProfile}`);
...
async function getProductsFromUser(ctx) {
  ...
  const { isCooking, isLactoseIntolerant,
    isVegetarien, isVegetalien,
    falseAllowedProductIds, falseForbiddenProductIds } = await ctx.db.query.user({where: {id: userId}}, `{
  ...
  // Remove false allowed products, false positive
  allowedProducts = _.filter(allowedProducts, function removeFalseAllowedProducts(product) {
